 <?php
    $certifications = [
        [
            "name" => "Advanced Adventurer",
            "code" => "AA",
            'cost' => 43.80,
            "price" => 250.00,
            "discounted_price" => 210.00,
            "is_speciality" => false,
            "own_speciality" => true,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Wreck Diving",
            "code" => "WD",
            'cost' => 43.80,
            "price" => 215.00,
            "discounted_price" => 160.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],

                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Discesa controllata sul relitto"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Orientamento sul relitto",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Navigazione attraverso il relitto",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Uso di una cima di sicurezza - Uso di una cima di corrente",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Risalita controllata con il compagno",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi facoltativi in acqua libera",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Lancio del segnalatore di superficie (SMB) in immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Discesa controllata sul relitto"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Orientamento sul relitto",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Navigazione attraverso il relitto",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Uso di una cima di sicurezza - Utilizzo di una cima guida con reel, per l'esplorazione",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Risalita controllata con il compagno",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi facoltativi in acqua libera",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Attraversamento di spazi chiusi"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Open Water Diver",
            "code" => "OWD",
            'cost' => 57.22,
            "price" => 370.00,
            "discounted_price" => 330.00,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione - Snorkeling",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assemblaggio e regolazione del Sistema Totale per snorkeling"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua - Snorkeling",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Valutazione dell'acquaticità"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Entrata con passo del gigante | Snorkeling",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Regolazione dell'assetto e della zavorra",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Controllo dell'assetto | Sistema Totale per Snorkeling",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Svuotamento dello snorkel e della maschera",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Pinneggiata alternata a pinne estese | In superficie",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Assetto positivo in superficie",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Tecniche di compensazione",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Capovolta a testa in giù",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Tecniche di pinneggiamento con lo snorkel | In immersione",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" =>
                                            "Ritorno in superficie",
                                        ],
                                        [
                                            "order" => 12,
                                            "label" =>
                                            "Uscita dall'acqua alta",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione - Snorkeling",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura | Snorkeling"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Assemblaggio dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Vestizione dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Check pre-immersione",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Entrata controllata da seduti",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Controllo dell'assetto | Sistema Totale d'Immersione",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Respirazione dall'erogatore",
                                        ],
                                        [
                                            "order" => 4,
                                            "label"
                                            => "Svuotamento dell'erogatore - Metodo dell'espirazione forzata",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Svuotamento dell'erogatore - Metodo del bottone di spurgo",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Recupero dell'erogatore - Circonduzione del braccio",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Recupero dell'erogatore - Metodo della fonte d'aria alternativa",
                                        ],

                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Svuotamento della maschera - Allagamento parziale",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Svuotamento della maschera - Allagamento totale",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Ripasso degli esercizi e tempo di pratica con supervisione",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" =>
                                            "Uscita",
                                        ],


                                    ]

                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Assemblaggio dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Vestizione dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Check pre-immersione",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Entrata con passo del gigante | Scuba",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Rimozione e Riposizionamento della Maschera",
                                        ],
                                        [
                                            "order" => 4,
                                            "label"
                                            => "Condivisione dell'aria sul posto - Passando l’erogatore primario",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Condivisione dell'aria sul posto - Uso della fonte d'aria alternativa",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Assetto Neutro | Posizione d'immersione - Gonfiaggio con VIS",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Assetto Neutro | Posizione d'immersione - Gonfiaggio a bocca",
                                        ],

                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Tecniche di pinneggiamento | Scuba",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Ripasso degli esercizi e tempo di pratica con supervisione",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Risalita controllata",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" =>
                                            "Uscita in acqua alta",
                                        ],
                                    ]

                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Cura dell'attrezzatura | Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 4,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Assemblaggio dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Vestizione dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Check pre-immersione",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Entrata con passo del gigante | Scuba",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Rimozione e riposizionamento della zavorra | In superficie",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Discesa controllata",
                                        ],
                                        [
                                            "order" => 4,
                                            "label"
                                            => "Rimozione e riposizionamento della zavorra | In immersione",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Rimozione e Riposizionamento della Maschera",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Assetto Neutro | Posizione d'immersione",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Condivisione dell'aria in risalita",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Ripasso degli esercizi e tempo di pratica con supervisione",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Risalita controllata",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Uscita",
                                        ],
                                    ]

                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Cura dell'attrezzatura | Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 5,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Assemblaggio dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Vestizione dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Check pre-immersione",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Entrata per rovesciamento all’indietro",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Rimozione e riposizionamento dell'unità Scuba | In immersione",
                                        ],
                                        [
                                            "order" => 4,
                                            "label"
                                            => "Assetto Neutro | Assetto neutro, in orizzontale",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Nuoto senza maschera",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Risalita di emergenza nuotando",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Ripasso degli esercizi e tempo di pratica con supervisione",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Rimozione e riposizionamento dell'unità Scuba | In superficie",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Uscita",
                                        ],
                                    ]

                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Cura dell'attrezzatura | Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 6,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Assemblaggio dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Vestizione dell'unità Scuba",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Check pre-immersione",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Entrata",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Disconnessione/riconnessione della frusta del VIS",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Respirazione da un erogatore in erogazione continua",
                                        ],
                                        [
                                            "order" => 4,
                                            "label"
                                            => "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Risalita di emergenza in assetto (REA)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Ripasso degli esercizi e tempo di pratica con supervisione",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Lancio in immersione di un segnalatore di superficie | In superficie",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Esercizi di soccorso - Avvicinamento",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Esercizi di soccorso - Rimozione di un crampo",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Esercizi di soccorso - Traino dalla rubinetteria",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" =>
                                            "Esercizi di soccorso - Traino dal fianco",
                                        ],
                                        [
                                            "order" => 12,
                                            "label" =>
                                            "Esercizi di soccorso - Spinta dalle pinne",
                                        ],
                                        [
                                            "order" => 13,
                                            "label" =>
                                            "Uscita",
                                        ],
                                    ]

                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Cura dell'attrezzatura | Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing e ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Vestizione dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua libera",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Entrata controllata da seduti"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo dell'assetto | Sistema Totale d'Immersione",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto Neutro | Posizione d'immersione",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Recupero dell'erogtore - Circonduzione del braccio",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Recupero dell'erogtore - Metodo della fonte d'aria alternativa",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Svuotamento della maschera - Allagamento parziale",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Svuotamento della maschera - Allagamento totale",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Risalita controllata",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" =>
                                            "Uscita",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing e ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Vestizione dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua libera",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Entrata per rovesciamento"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto Neutro | Posizione d'immersione",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Rimozione e Riposizionamento della Maschera",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Condivisione dell'aria sul posto",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Risalita controllata",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Passaggio da snorkel a erogatore e viceversa",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Nuoto in superficie",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Uscita",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing e ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Vestizione dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua libera",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Entrata con passo del gigante | Scuba"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto Neutro | Posizione d'immersione",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Rimozione e Riposizionamento della Maschera",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Condivisione dell'aria in risalita",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Rimozione e riposizionamento della zavorra | In superficie",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Uscita",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi facoltativi in acqua libera",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Nuoto senza maschera"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 5,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 4,
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing e ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Vestizione dell'unità Scuba"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua libera",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Entrata"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa in libera",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto Neutro | Posizione d'immersione",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Rotta reciproca con la bussola",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Risalita controllata",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Rimozione e riposizionamento dell'unità Scuba | In superficie",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Lancio in immersione di un segnalatore di superficie | In superficie",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Uscita",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi facoltativi in acqua libera",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo del consumo di gas"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Risalita di emergenza nuotando"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 5,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 5,
                            'label' => "Esercizi Snorkeling | Esercizi Scuba di soccorso",
                            'values' => [
                                [
                                    "label" => "Esercizi prima dell'immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Vestizione del Sistema Totale d’Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Regolazione dell'assetto e della zavorra"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua libera",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Svuotamento dello snorkel e della maschera"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Ritrovamento e sgancio della zavorra",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto positivo in superficie",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" =>
                                            "Esercizi di soccorso - Avvicinamento",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Esercizi di soccorso - Rimozione di un crampo",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Esercizi di soccorso - Traino dalla rubinetteria",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Esercizi di soccorso - Traino dal fianco",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Esercizi di soccorso - Spinta dalle pinne",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Ritorno in superficie",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Uscita dall'acqua alta",
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità Scuba"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ],
        ],
        [
            "name" => "Coral Identification",
            "code" => "CI",
            'cost' => 43.8,
            "price" => 175.00,
            "discounted_price" => 100.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],

                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Pianificazione dell'immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Identificazione della fauna in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Idenfificare quante più specie o singoli individui possibile"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Idenfificare qualsiasi caratteristica significativa o identificativa"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Idenfificare qualsiasi comportamento (caccia, costruzione di un nido, accoppiamenti, ecc.)"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Idenfificare qualsiasi comportamento particolare o unico nel suo genere"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Idenfificare l'habitat o l'ecosistema in cui un dato animale vive"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi post-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Dimostrazione a secco della navigazione su rotte complesse"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Correzione per la corrente (facoltativa)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Rotte bussola complesse - Rotta a 3 lati (3 deviazioni di 120° ciascuna)",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Rotte bussola complesse - Rotta a 4 lati (4 deviazioni di 90° ciascuna)",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Rotte per posizione stimata",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Decompression Diving",
            "code" => "DD",
            'cost' => 43.80,
            "price" => 215.00,
            "discounted_price" => 160.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Rimozione e riposizionamento della bombola di fase (in superficie)"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Esercizio S",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Controllo dell'Indice di Respirazione in Superficie (SAC)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Rimozione e riposizionamento della bombola di fase (in immersione)",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Passaggio al gas decompressivo",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Condivisione del gas in risalita, con cambio gas.",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Lancio di un segnalatore di superficie (DSMB) in immersione",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Revisione del piano d'immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Rimozione e riposizionamento della bombola di fase (in superficie)"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Esercizio S",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Rimozione e riposizionamento della bombola di fase (in immersione)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Condivisione del gas in risalita, con cambio gas.",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Condurre una decompressione (simulata)",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Lancio di un segnalatore di superficie (DSMB) in immersione",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Revisione del piano d'immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Esercizio S",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Condurre una decompressione",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" =>
                                            "Lancio di un segnalatore di superficie (DSMB) in immersione",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Revisione del piano d'immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Esercizio S",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Condurre una decompressione",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" =>
                                            "Lancio di un segnalatore di superficie (DSMB) in immersione",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Deep Diving",
            "code" => "DDS",
            'cost' => 43.80,
            "price" => 215.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "discounted_price" => 160.00,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale Controllo dell'assetto, con indossato il Sistema Totale d'Immersione "
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Adattamento dell'attrezzatura (in immersione)",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Lancio del segnalatore di superficie (SMB) in immersione",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Risalita controllata con il compagno",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Indice di respirazione in superficie (SAC)",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale Controllo dell'assetto, con indossato il Sistema Totale d'Immersione "
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Adattamento dell'attrezzatura (in immersione)",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Lancio del segnalatore di superficie (SMB) in immersione",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Risalita controllata con il compagno",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Indice di respirazione in superficie (SAC)",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale Controllo dell'assetto, con indossato il Sistema Totale d'Immersione "
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Adattamento dell'attrezzatura (in immersione)",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Lancio del segnalatore di superficie (SMB) in immersione",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Risalita controllata con il compagno",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Indice di respirazione in superficie (SAC)",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Dry Suit Diving",
            "code" => "DSD",
            'cost' => 43.80,
            "price" => 215.00,
            "discounted_price" => 160.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione dell'unità SCUBA con indosso la stagna"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto con il Sistema Totale d'Immersione completo, con indosso la stagna"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata (Muta stagna)",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Confidenza con la stagna",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Esercizi con le valvole della stagna - Esercizi di malfunzionamento della valvola",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Subacqueo a testa in giù",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Eccessivo gonfiaggio di una muta stagna",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Simulazione dell'allagamento della stagna",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Risalita controllata (Muta stagna)",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (Muta stagna)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione dell'unità SCUBA con indosso la stagna"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto con il Sistema Totale d'Immersione completo, con indosso la stagna"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata (Muta stagna)",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Confidenza con la stagna",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Esercizi con le valvole della stagna - Esercizi di malfunzionamento della valvola",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Subacqueo a testa in giù",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Eccessivo gonfiaggio di una muta stagna",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Simulazione dell'allagamento della stagna",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Risalita controllata (Muta stagna)",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (Muta stagna)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione dell'unità SCUBA con indosso la stagna"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto con il Sistema Totale d'Immersione completo, con indosso la stagna"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata (Muta stagna)",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Confidenza con la stagna",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Esercizi con le valvole della stagna - Esercizi di malfunzionamento della valvola",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Subacqueo a testa in giù",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Eccessivo gonfiaggio di una muta stagna",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Simulazione dell'allagamento della stagna",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Risalita controllata (Muta stagna)",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (Muta stagna)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Enriched Air Nitrox",
            "code" => "EAN",
            'cost' => 43.80,
            "price" => 175.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "discounted_price" => 130.00,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Analisi della bombola",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Analizzare la bombola autonomamente"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Controllo della taratura dello strumento",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Calibrazione dell'analizzatore, con una fonte di gas conosciuta, e analisi di una o più bombole"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Etichettatura della bombola con il suo contenuto e registrazione dei risulatati nel registro della stazione di ricarica",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Analisi della bombola",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Analizzare la bombola autonomamente"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Controllo della taratura dello strumento",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Calibrazione dell'analizzatore, con una fonte di gas conosciuta, e analisi di una o più bombole"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Etichettatura della bombola con il suo contenuto e registrazione dei risulatati nel registro della stazione di ricarica",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Perfect Boyancy",
            "code" => "PB",
            'cost' => 43.80,
            "price" => 215.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "discounted_price" => 160.00,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'unità SCUBA"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Respirazione rilassata",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto Neutro | Posizione d'immersione - Gonfiaggio con VIS",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Assetto Neutro | Posizione d'immersione - Gonfiaggio a bocca",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Assetto Neutro | Assetto neutro, in orizzontale",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Spostamenti sott'acqua",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Rotta subacquea con ostacolo",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Esercizi con nuova attrezzatura",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'unità SCUBA"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Respirazione rilassata",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto Neutro | Posizione d'immersione - Gonfiaggio con VIS",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Assetto Neutro | Posizione d'immersione - Gonfiaggio a bocca",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Assetto Neutro | Assetto neutro, in orizzontale",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Spostamenti sott'acqua",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Rotta subacquea con ostacolo",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Esercizi con nuova attrezzatura",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'unità SCUBA"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Respirazione rilassata",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto Neutro | Posizione d'immersione - Gonfiaggio con VIS",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Assetto Neutro | Posizione d'immersione - Gonfiaggio a bocca",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Assetto Neutro | Assetto neutro, in orizzontale",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Spostamenti sott'acqua",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Rotta subacquea con ostacolo",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Esercizi con nuova attrezzatura",
                                        ],

                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Equipment Techniques",
            "code" => "ET",
            'cost' => 43.80,
            "price" => 135.00,
            "discounted_price" => 100.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Fish Identification",
            "code" => "FI",
            'cost' => 36.48,
            "price" => 135.00,
            "discounted_price" => 100.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],

                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Pianificazione dell'immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Identificazione della fauna in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Idenfificare quante più specie o singoli individui possibile"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Idenfificare qualsiasi caratteristica significativa o identificativa"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Idenfificare qualsiasi comportamento (caccia, costruzione di un nido, accoppiamenti, ecc.)"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Idenfificare qualsiasi comportamento particolare o unico nel suo genere"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Idenfificare l'habitat o l'ecosistema in cui un dato animale vive"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Marine Ecology",
            "code" => "ME",
            'cost' => 36.48,
            "price" => 135.00,
            "discounted_price" => 100.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],

                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Pianificazione dell'immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Identificazione della fauna in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Idenfificare quante più specie o singoli individui possibile"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Idenfificare qualsiasi caratteristica significativa o identificativa"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Idenfificare qualsiasi comportamento (caccia, costruzione di un nido, accoppiamenti, ecc.)"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Idenfificare qualsiasi comportamento particolare o unico nel suo genere"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Idenfificare l'habitat o l'ecosistema in cui un dato animale vive"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi post-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Dimostrazione a secco della navigazione su rotte complesse"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Correzione per la corrente (facoltativa)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Rotte bussola complesse - Rotta a 3 lati (3 deviazioni di 120° ciascuna)",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Rotte bussola complesse - Rotta a 4 lati (4 deviazioni di 90° ciascuna)",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Rotte per posizione stimata",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Navigation",
            "code" => "NAV",
            'cost' => 43.80,
            "price" => 215.00,
            "discounted_price" => 160.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Dimostrazione a secco della navigazione naturale e delle rotte reciproche"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Dimostrazione a secco della navigazione su rotte complesse"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Correzione per la corrente (facoltativa)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Determinazione della velocità di nuoto"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Navigazione naturale",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Rotta reciproca con la bussola",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Combinazione di navigazione naturale e con la bussola",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Rotte bussola complesse - Rotta a 3 lati (3 deviazioni di 120° ciascuna)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Rotte bussola complesse - Rotta a 4 lati (4 deviazioni di 90° ciascuna)",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Rotte per posizione stimata",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Dimostrazione a secco della navigazione naturale e delle rotte reciproche"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Determinazione della velocità di nuoto"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Navigazione naturale",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Rotta reciproca con la bussola",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Combinazione di navigazione naturale e con la bussola",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Dimostrazione a secco della navigazione su rotte complesse"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Correzione per la corrente (facoltativa)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" =>
                                            "Rotte bussola complesse - Rotta a 3 lati (3 deviazioni di 120° ciascuna)",
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Rotte bussola complesse - Rotta a 4 lati (4 deviazioni di 90° ciascuna)",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" =>
                                            "Rotte per posizione stimata",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Night Diving and Limited Visibility",
            "code" => "NLV",
            'cost' => 43.80,
            "price" => 215.00,
            "discounted_price" => 160.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],

                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],


                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Mantenimento del contatto con il compagno",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Rotta reciproca con la bussola"
                                        ],
                                        [
                                            "order" => 6,
                                            "label" => "Blackout della torcia",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" => "Procedura per la perdita del compagno",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" => "Risalita controllata con il compagno",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],


                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Mantenimento del contatto con il compagno",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Rotta reciproca con la bussola"
                                        ],
                                        [
                                            "order" => 6,
                                            "label" => "Blackout della torcia",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" => "Procedura per la perdita del compagno",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" => "Risalita controllata con il compagno",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Science Of Diving",
            "code" => "SOD",
            'cost' => 36.48,
            "price" => 135.00,
            "discounted_price" => 135.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Scuba Skills Update",
            "code" => "SSU",
            'cost' => 12.08,
            "price" => 80.00,
            "discounted_price" => 60.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'CW',
                    'order' => 1,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing, check pre-immersione e segnali manuali"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'unità SCUBA"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Vestizione dell'unità SCUBA"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Entrate ed uscite"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Discesa controllata",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Svuotamento della maschera",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Svuotamento dell'erogatore (espirando e con bottone di spurgo)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Recupero dell'erogatore (Circonduzione del braccio e fonte d'aria alternativa)",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Condivisione dell'aria sul posto",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Assetto neutro (nella posizione d'immersione)",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Risalita controllata con il compagno",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Condivisione dell'aria in risalita",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" =>
                                            "Esercizi: Risalite di emergenza",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Smontaggio dell'unità SCUBA"
                                        ],
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (SCUBA)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],


            ]

        ],
        [
            "name" => "Search & Recovery",
            "code" => "SR",
            'cost' => 43.80,
            "price" => 215.00,
            "discounted_price" => 160.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],

                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Entrata in acqua"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Spiegamento di una cima guida"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Schema di ricerca circolare (adatto ad oggetti medio/grandi)"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Schema a rettangolo espanso (adatto ad oggetti medio/grandi)"
                                        ],
                                        [
                                            "order" => 6,
                                            "label" => "Schema a griglia (adatto ad oggetti piccoli o sepolti)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Entrata in acqua"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Spiegamento di una cima guida"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Schema di ricerca circolare (adatto ad oggetti medio/grandi)"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Schema a rettangolo espanso (adatto ad oggetti medio/grandi)"
                                        ],
                                        [
                                            "order" => 6,
                                            "label" => "Schema a griglia (adatto ad oggetti piccoli o sepolti)"
                                        ],
                                        [
                                            "order" => 7,
                                            "label" => "Segnalazione dell'oggetto"
                                        ],
                                        [
                                            "order" => 8,
                                            "label" => "Uso del pallone di sollevamento"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Stress & Rescue",
            "code" => "DSR",
            'cost' => 43.80,
            "price" => 420.00,
            "discounted_price" => 350.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi di livello iniziale",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Recupero dell’erogatore | Circonduzione del braccio e fonte d’aria alternativa"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Condivisione dell'aria sul posto"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Rimozione e riposizionamento dell'unità Scuba | In superficie"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Rimozione e riposizionamento dell'unità Scuba | In immersione"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Rimozione e riposizionamento della zavorra | In superficie"
                                        ],
                                        [
                                            "order" => 6,
                                            "label" => "Rimozione e riposizionamento della zavorra | In immersione"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi di autosoccorso",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Rimozione di un crampo"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Nuoto senza maschera"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Respirazione da un erogatore in erogazione continua"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Disconnessione/Riconnessione della frusta di carico del GAV"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi di risalita di emergenza",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Risalita di emergenza nuotando"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Risalita di emergenza in assetto (REA)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 5,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Assistenza al subacqueo",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assistenza al subacqueo - In immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assistenza al subacqueo - In superficie"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Traino di un subacqueo affaticato",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Traino dalla rubinetteria"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Spinta dalle pinne"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Traino dal fianco"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Traino utilizzando un sistema di galleggiamento"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Soccorso di un subacqueo",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Raggiungere l'assistenza"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assistenza ad un subacqueo in difficoltà"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assistenza a un subacqueo in panico"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Inizio della catena di soccorso"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Condivisione dell'aria in risalita"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 5,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Ricerca di un subacqueo disperso"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Subacqueo privo di sensi sott'acqua"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Subacqueo privo di sensi in superficie"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Trasporto di un subacqueo privo di sensi"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Rimozione di un subacqueo privo di sensi dall'acqua"
                                        ],
                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Scenario di soccorso 1 | Gestione dello stress",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Un subacqueo ha gli occhi spalancati e respira rapidamente"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Un subacqueo ha problemi di controllo dell'assetto"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Un subacqueo ha problemi con l'attrezzatura"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Un subacqueo ha un problema fisico"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Scenario di soccorso 2 | Assistenza di un subacqueo cosciente",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Un subacqueo è in difficoltà o in preda al panico in superficie"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Un subacqueo è cosciente in superficie e richiede assistenza medica"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Un subacqueo è in difficoltà o in preda al panico sott'acqua e deve condividere il gas respiratorio"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Scenario di soccorso 3 | Soccorso di un subacqueo privo di sensi",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Un subacqueo viene trovato svenuto sott'acqua"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Un subacqueo è disperso e viene trovato privo di sensi sott'acqua"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Try Scuba",
            "code" => "TS",
            'cost' => 0.00,
            "price" => 50.00,
            "discounted_price" => 0.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],

                    ]
                ],

                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing, Controllo pre-immersione, e ripasso dei segnali manuali"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Regolazione dell'attrezzatura"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua - Acqua bassa",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Entrate ed uscite"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Respirazione dall'erogatore",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Svuotamento dell'erogatore - Metodo dell'espirazione forzata",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Svuotamento dell'erogatore - Metodo del bottone di spurgo",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Recupero dell'erogatore - Circonduzione del braccio",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" => "Recupero dell'erogatore - Metodo della fonte d'aria alternativa",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" => "Svuotamento della maschera",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" => "Condivisione dell'aria sul posto | Con l'Istruttore",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" => "Tecniche di compensazione",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua - Acqua alta",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto Neutro | Posizione d'immersione"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Risalita controllata"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Discesa controllata sul relitto"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Orientamento sul relitto",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Navigazione attraverso il relitto",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Uso di una cima di sicurezza - Utilizzo di una cima guida con reel, per l'esplorazione",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Risalita controllata con il compagno",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi facoltativi in acqua libera",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Attraversamento di spazi chiusi"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Waves, Tides & Currents",
            "code" => "WTC",
            'cost' => 43.80,
            "price" => 215.00,
            "discounted_price" => 160.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],

                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione dell'unità SCUBA"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Tecniche di entrata ed uscita"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa in libera",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto neutro (nella posizione d'immersione)",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Risalita controllata con il compagno",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Lancio del segnalatore di superficie (SMB) in immersione",
                                        ],


                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (SCUBA)"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione dell'unità SCUBA"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Tecniche di entrata ed uscita"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa in libera",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto neutro (nella posizione d'immersione)",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Risalita controllata con il compagno",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Sosta di sicurezza",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Lancio del segnalatore di superficie (SMB) in immersione",
                                        ],


                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (SCUBA)"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Gas Blender Nitrox/Trimix",
            "code" => "GB",
            'cost' => 43.80,
            "price" => 175.00,
            "discounted_price" => 130.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Indipendent Diving",
            "code" => "ID",
            'cost' => 43.80,
            "price" => 175.00,
            "discounted_price" => 130.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Controllo pre-immerisone"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Ingresso in acqua"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo pesata"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Bobbing - Gonfiaggio a bocca del GAV"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Rimozione e riposizionamento della zavorra | In superficie"
                                        ],
                                        [
                                            "order" => 5,
                                            "Rimozione e riposizionamento dell'unità Scuba | In superficie",
                                        ],
                                        [
                                            "order" => 6,
                                            "Procedure di superficie",
                                        ],
                                        [
                                            "order" => 7,
                                            "Discesa controllata",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" => "Svuotamento dell'erogatore - Metodo dell'espirazione forzata",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" => "Svuotamento dell'erogatore - Metodo del bottone di spurgo",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" => "Recupero dell'erogatore - Circonduzione del braccio",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" => "Recupero dell'erogatore - Metodo della fonte d'aria alternativa",
                                        ],
                                        [
                                            "order" => 12,
                                            "Respirazione da erogatore in continua",
                                        ],
                                        [
                                            "order" => 13,
                                            "Cambio bombola e respirazione da una bombola secondaria",
                                        ],
                                        [
                                            "order" => 14,
                                            "label" => "Mantenimento dell assetto sott'acqua - Gonfiaggio GAV con il VIS",
                                        ],
                                        [
                                            "order" => 15,
                                            "label" => "Mantenimento dell assetto sott'acqua - Gonfiaggio GAV a bocca",
                                        ],
                                        [
                                            "order" => 16,
                                            "laber" => "Disconnessione/riconnessione della frusta del VIS",
                                        ],
                                        [
                                            "order" => 17,
                                            "laber" => "Rimozione e riposizionamento dell'unità Scuba | In immersione",
                                        ],
                                        [
                                            "order" => 18,
                                            "label" => "Nuoto senza maschera",
                                        ],
                                        [
                                            "order" => 19,
                                            "laber" =>  "Risalita di emergenza in assetto (REA)",
                                        ],
                                        [
                                            "order" => 20,
                                            "label" => "Risalita di emergenza nuotando (REN)"
                                        ],
                                        [
                                            "order" => 21,
                                            "label" => "Lancio di un segnalatore di superficie (DSMB) in immersione",
                                        ],
                                        [
                                            "order" => 22,
                                            "label" => "Rimozione dei crampi",
                                        ],
                                        [
                                            "order" => 23,
                                            "label" => "Risalita controllata",
                                        ],
                                        [
                                            "order" => 24,
                                            "label" => "Uscita",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi post-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],

                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Controllo pre-immerisone"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Ingresso in acqua"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo pesata"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Bobbing - Gonfiaggio a bocca del GAV"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Rimozione e riposizionamento della zavorra | In superficie"
                                        ],
                                        [
                                            "order" => 5,
                                            "Rimozione e riposizionamento dell'unità Scuba | In superficie",
                                        ],
                                        [
                                            "order" => 6,
                                            "Procedure di superficie",
                                        ],
                                        [
                                            "order" => 7,
                                            "Discesa controllata",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" => "Svuotamento dell'erogatore - Metodo dell'espirazione forzata",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" => "Svuotamento dell'erogatore - Metodo del bottone di spurgo",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" => "Recupero dell'erogatore - Circonduzione del braccio",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" => "Recupero dell'erogatore - Metodo della fonte d'aria alternativa",
                                        ],
                                        [
                                            "order" => 12,
                                            "Respirazione da erogatore in continua",
                                        ],
                                        [
                                            "order" => 13,
                                            "Cambio bombola e respirazione da una bombola secondaria",
                                        ],
                                        [
                                            "order" => 14,
                                            "label" => "Mantenimento dell assetto sott'acqua - Gonfiaggio GAV con il VIS",
                                        ],
                                        [
                                            "order" => 15,
                                            "label" => "Mantenimento dell assetto sott'acqua - Gonfiaggio GAV a bocca",
                                        ],
                                        [
                                            "order" => 16,
                                            "laber" => "Disconnessione/riconnessione della frusta del VIS",
                                        ],

                                        [
                                            "order" => 17,
                                            "label" => "Nuoto senza maschera",
                                        ],

                                        [
                                            "order" => 18,
                                            "label" => "Risalita di emergenza nuotando (REN)"
                                        ],
                                        [
                                            "order" => 19,
                                            "label" => "Lancio di un segnalatore di superficie (DSMB) in immersione",
                                        ],
                                        [
                                            "order" => 20,
                                            "label" => "Rimozione dei crampi",
                                        ],
                                        [
                                            "order" => 21,
                                            "label" => "Risalita controllata",
                                        ],
                                        [
                                            "order" => 22,
                                            "label" => "Uscita",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi post-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Controllo pre-immerisone"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Ingresso in acqua"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo pesata"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Bobbing - Gonfiaggio a bocca del GAV"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Rimozione e riposizionamento della zavorra | In superficie"
                                        ],
                                        [
                                            "order" => 5,
                                            "Rimozione e riposizionamento dell'unità Scuba | In superficie",
                                        ],
                                        [
                                            "order" => 6,
                                            "Procedure di superficie",
                                        ],
                                        [
                                            "order" => 7,
                                            "Discesa controllata",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" => "Svuotamento dell'erogatore - Metodo dell'espirazione forzata",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" => "Svuotamento dell'erogatore - Metodo del bottone di spurgo",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" => "Recupero dell'erogatore - Circonduzione del braccio",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" => "Recupero dell'erogatore - Metodo della fonte d'aria alternativa",
                                        ],
                                        [
                                            "order" => 12,
                                            "Respirazione da erogatore in continua",
                                        ],
                                        [
                                            "order" => 13,
                                            "Cambio bombola e respirazione da una bombola secondaria",
                                        ],
                                        [
                                            "order" => 14,
                                            "label" => "Mantenimento dell assetto sott'acqua - Gonfiaggio GAV con il VIS",
                                        ],
                                        [
                                            "order" => 15,
                                            "label" => "Mantenimento dell assetto sott'acqua - Gonfiaggio GAV a bocca",
                                        ],
                                        [
                                            "order" => 16,
                                            "laber" => "Disconnessione/riconnessione della frusta del VIS",
                                        ],

                                        [
                                            "order" => 17,
                                            "label" => "Nuoto senza maschera",
                                        ],

                                        [
                                            "order" => 18,
                                            "label" => "Risalita di emergenza nuotando (REN)"
                                        ],
                                        [
                                            "order" => 19,
                                            "label" => "Lancio di un segnalatore di superficie (DSMB) in immersione",
                                        ],
                                        [
                                            "order" => 20,
                                            "label" => "Rimozione dei crampi",
                                        ],
                                        [
                                            "order" => 21,
                                            "label" => "Risalita controllata",
                                        ],
                                        [
                                            "order" => 22,
                                            "label" => "Uscita",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi post-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Briefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Controllo pre-immerisone"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Ingresso in acqua"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo pesata"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Bobbing - Gonfiaggio a bocca del GAV"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Rimozione e riposizionamento della zavorra | In superficie"
                                        ],
                                        [
                                            "order" => 5,
                                            "Rimozione e riposizionamento dell'unità Scuba | In superficie",
                                        ],
                                        [
                                            "order" => 6,
                                            "Procedure di superficie",
                                        ],
                                        [
                                            "order" => 7,
                                            "Discesa controllata",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" => "Svuotamento dell'erogatore - Metodo dell'espirazione forzata",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" => "Svuotamento dell'erogatore - Metodo del bottone di spurgo",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" => "Recupero dell'erogatore - Circonduzione del braccio",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" => "Recupero dell'erogatore - Metodo della fonte d'aria alternativa",
                                        ],
                                        [
                                            "order" => 12,
                                            "Respirazione da erogatore in continua",
                                        ],
                                        [
                                            "order" => 13,
                                            "Cambio bombola e respirazione da una bombola secondaria",
                                        ],
                                        [
                                            "order" => 14,
                                            "label" => "Mantenimento dell assetto sott'acqua - Gonfiaggio GAV con il VIS",
                                        ],
                                        [
                                            "order" => 15,
                                            "label" => "Mantenimento dell assetto sott'acqua - Gonfiaggio GAV a bocca",
                                        ],
                                        [
                                            "order" => 16,
                                            "laber" => "Disconnessione/riconnessione della frusta del VIS",
                                        ],

                                        [
                                            "order" => 17,
                                            "label" => "Nuoto senza maschera",
                                        ],

                                        [
                                            "order" => 18,
                                            "label" => "Risalita di emergenza nuotando (REN)"
                                        ],
                                        [
                                            "order" => 19,
                                            "label" => "Lancio di un segnalatore di superficie (DSMB) in immersione",
                                        ],
                                        [
                                            "order" => 20,
                                            "label" => "Rimozione dei crampi",
                                        ],
                                        [
                                            "order" => 21,
                                            "label" => "Risalita controllata",
                                        ],
                                        [
                                            "order" => 22,
                                            "label" => "Uscita",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi post-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Dive Guide ",
            "code" => "DG",
            'cost' => 110.90,
            "price" => 500.00,
            "discounted_price" => 400.00
        ],

        [
            "name" => "Snorkel Diver",
            "code" => "SND",
            'cost' => 0.00,
            "price" => 50.00,
            "discounted_price" => 40.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'CW',
                    'order' => 1,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assemblaggio e preparazione dell'attrezzatura Snorkeling"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione del Sistema Totale per Snorkeling"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Adattamento dell'attrezzatura (in immersione)"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Regolazione dell'assetto e della zavorra"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo dell'assetto con un sistema Totale per Snorkeling",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Svuotamento dello snorkel e della maschera",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Pinneggiata alternata a pinne estese (in superficie)",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Ottenimento dell'assetto positivo in superficie, facendo snorkeling",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Tecniche di compensazione",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Capovolta a testa in giù",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Ritorno in superficie",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Entrate ed uscite",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Ottenimento dell'assetto positivo in superficie (Gonfiaggio a bocca con bobbing)",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (Snorkeling))"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assemblaggio e preparazione dell'attrezzatura Snorkeling"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione del Sistema Totale per Snorkeling"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Adattamento dell'attrezzatura (in immersione)"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Regolazione dell'assetto e della zavorra"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Controllo dell'assetto con un sistema Totale per Snorkeling",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Svuotamento dello snorkel e della maschera",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Pinneggiata alternata a pinne estese (in superficie)",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Ottenimento dell'assetto positivo in superficie, facendo snorkeling",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Tecniche di compensazione",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Capovolta a testa in giù",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Ritorno in superficie",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Entrate ed uscite",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Ottenimento dell'assetto positivo in superficie (Gonfiaggio a bocca con bobbing)",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (Snorkeling))"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
        [
            "name" => "Try Freediving",
            "code" => "TF",
            'cost' => 0.00,
            "price" => 50.00,
            "discounted_price" => 0.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [

                [
                    'label' => 'CW',
                    'order' => 1,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,

                                    [
                                        "order" => 1,
                                        "label" => "Preparazione dell'attrezzatura"
                                    ],

                                ]
                            ],
                            [
                                "label" => "Esercizi in acqua",
                                "order" => 2,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Svuotamento snorkel"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Respirazione diaframamtica",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Respirazione di recupero",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Apnea statica",
                                    ],
                                    [
                                        "order" => 5,
                                        "label" => "Apnea dinamica e assetto neutro",
                                    ],

                                    [
                                        "order" => 6,
                                        "label" => "Compensazione",
                                    ],
                                    [
                                        "order" => 7,
                                        "label" => "Capovolta",
                                    ]


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Freediving))"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ],

                    ],

                ],
            ]
        ],
        [
            "name" => "Basic Freediving",
            "code" => "BF",
            'cost' => 23.06,
            "price" => 150.00,
            "discounted_price" => 120.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,

                                    [
                                        "order" => 1,
                                        "label" => "Preparazione dell'attrezzatura"
                                    ],

                                ]
                            ],
                            [
                                "label" => "Esercizi in acqua",
                                "order" => 2,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Svuotamento snorkel"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Respirazione diaframamtica",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Respirazione di recupero",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Apnea statica",
                                    ],
                                    [
                                        "order" => 5,
                                        "label" => "Apnea dinamica e assetto neutro",
                                    ],
                                    [
                                        "order" => 6,
                                        "label" => "Tecniche di pinneggiamento",
                                    ],
                                    [
                                        "order" => 7,
                                        "label" => "Posizione del corpo idrodinamica",
                                    ],
                                    [
                                        "order" => 8,
                                        "label" => "Rimozione dei crampi",
                                    ],
                                    [
                                        "order" => 9,
                                        "label" => "Assetto positivo in superficie",
                                    ],
                                    [
                                        "order" => 10,
                                        "label" => "Sincope Soccorso-Risposta-Salvataggio",
                                    ],
                                    [
                                        "order" => 11,
                                        "label" => "Compensazione",
                                    ],
                                    [
                                        "order" => 12,
                                        "label" => "Capovolta",
                                    ]


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Freediving))"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,

                                    [
                                        "order" => 1,
                                        "label" => "Preparazione dell'attrezzatura"
                                    ],

                                ]
                            ],
                            [
                                "label" => "Esercizi in acqua",
                                "order" => 2,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Svuotamento snorkel"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Respirazione diaframamtica",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Respirazione di recupero",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Apnea statica",
                                    ],
                                    [
                                        "order" => 5,
                                        "label" => "Apnea dinamica e assetto neutro",
                                    ],
                                    [
                                        "order" => 6,
                                        "label" => "Tecniche di pinneggiamento",
                                    ],
                                    [
                                        "order" => 7,
                                        "label" => "Posizione del corpo idrodinamica",
                                    ],
                                    [
                                        "order" => 8,
                                        "label" => "Rimozione dei crampi",
                                    ],
                                    [
                                        "order" => 9,
                                        "label" => "Assetto positivo in superficie",
                                    ],
                                    [
                                        "order" => 10,
                                        "label" => "Sincope Soccorso-Risposta-Salvataggio",
                                    ],
                                    [
                                        "order" => 11,
                                        "label" => "Compensazione",
                                    ],
                                    [
                                        "order" => 12,
                                        "label" => "Capovolta",
                                    ]


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Freediving))"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ],
                    ],

                ],
            ]
        ],
        [
            "name" => "Try Mermaid",
            "code" => "TM",
            'cost' => 0.00,
            "price" => 50.00,
            "discounted_price" => 0.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [

                [
                    'label' => 'CW',
                    'order' => 1,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,

                                    [
                                        "order" => 1,
                                        "label" => "Vestizione dell'attrezzatura Mermaid"
                                    ],

                                ]
                            ],
                            [
                                "label" => "Esercizi in acqua",
                                "order" => 2,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Entrata da seduti"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Nuoto a dorso",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Pinneggiata con monopinna (in superficie)",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Assistenza al compagno - Rimozione del crampo",
                                    ],
                                    [
                                        "order" => 5,
                                        "label" => "Assistenza al compagno - Trasporto del compagno stanco",
                                    ],


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Mermaid)"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ],
                    ],

                ],

            ]
        ],
        [
            "name" => "Mermaid",
            "code" => "MR",
            'cost' => 35.26,
            "price" => 180.00,
            "discounted_price" => 150.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,

                                    [
                                        "order" => 1,
                                        "label" => "Vestizione dell'attrezzatura Mermaid"
                                    ],

                                ]
                            ],
                            [
                                "label" => "Esercizi in acqua",
                                "order" => 2,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Entrata da seduti"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Nuoto a dorso",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Pinneggiata con monopinna (in superficie)",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Assistenza al compagno - Rimozione del crampo",
                                    ],
                                    [
                                        "order" => 5,
                                        "label" => "Assistenza al compagno - Trasporto del compagno stanco",
                                    ],


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Mermaid)"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,

                                    [
                                        "order" => 1,
                                        "label" => "Vestizione dell'attrezzatura Mermaid"
                                    ],

                                ]
                            ],
                            [
                                "label" => "Esercizi in acqua",
                                "order" => 2,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Ripasso esercizi"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Respirazione di emergenza",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Trasporto di emergenza",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Pinneggiata a delfino",
                                    ],
                                    [
                                        "order" => 5,
                                        "label" => "Esercizi di divertimento",
                                    ],


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Mermaid)"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ],
                        [
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,

                                    [
                                        "order" => 1,
                                        "label" => "Vestizione dell'attrezzatura Mermaid"
                                    ],

                                ]
                            ],
                            [
                                "label" => "Esercizi in acqua",
                                "order" => 2,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Ripasso esercizi"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Capovolta a testa in giù",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Uscita dall'acqua alta",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Esercizi di divertimento",
                                    ],


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Mermaid)"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ]
                    ],

                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [

                            [
                                "label" => "Esercizi in acqua",
                                "order" => 1,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Entrata da seduti"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Entrata dalla spiaggia",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Vestizione dell'attrezzatura Mermaid (in superficie)",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Capovolta a testa in giù",
                                    ],
                                    [
                                        "order" => 5,
                                        "label" => "Pinneggiata a delfino",
                                    ],


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Mermaid)"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ],
                        [

                            [
                                "label" => "Esercizi in acqua",
                                "order" => 1,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Ripasso degli esercizi"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" => "Respirazione di emergenza",
                                    ],
                                    [
                                        "order" => 3,
                                        "label" => "Sincope Soccorso-Risposta-Salvataggio",
                                    ],
                                    [
                                        "order" => 4,
                                        "label" => "Trasporto di emergenza",
                                    ],


                                ]
                            ],
                            [
                                "label" => "Esercizi dopo l'immersione",
                                "order" => 3,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Cura dell'attrezzatura (Mermaid)"
                                    ],
                                ]
                            ],
                            [
                                "label" => "Debriefing",
                                "order" => 4,
                                "values" => [
                                    [
                                        "order" => 1,
                                        "label" => "Debriefing"
                                    ],
                                    [
                                        "order" => 2,
                                        "label" =>
                                        "Registrare e convalidare immersione su Logbook App SSI",
                                    ],
                                ]
                            ],
                        ],
                    ],
                ],




            ]
        ],
        [
            "name" => "React Right - BLS FA",
            "code" => "RR1",
            'cost' => 42.70,
            "price" => 200.00,
            "discounted_price" => 150.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Sessioni di apprendimento attivo',
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Valutazione della scena',
                                        ],
                                        [
                                            "order" => 2,
                                            'label' => 'Uso dei guanti e barriere protettivi',
                                        ],
                                        [
                                            "order" => 3,
                                            'label' => 'Avvicinamento alla vittima e controllo dello stato di coscienza',
                                        ],
                                        [
                                            "order" => 4,
                                            'label' => "Chiamata SME",
                                        ],
                                        [
                                            "order" => 5,
                                            'label' => "Posizione di sicurezza",
                                        ],
                                        [
                                            "order" => 6,
                                            'label' => "Emorraggia grave esterna e interna",
                                        ],
                                        [
                                            "order" => 7,
                                            'label' => "Ostruzione delle vie aeree",
                                        ],
                                        [
                                            "order" => 8,
                                            'label' => "Gestione dello shock",
                                        ],
                                        [
                                            "order" => 9,
                                            'label' => "Monitoraggio dei segni vitali",
                                        ],
                                        [
                                            "order" => 10,
                                            'label' => "Ustioni da calore e da elettricità",
                                        ],
                                        [
                                            "order" => 11,
                                            'label' => "Ipotermia",
                                        ],
                                        [
                                            "order" => 12,
                                            'label' => "Esaurimento da calore a colpo di calore",
                                        ],
                                        [
                                            "order" => 13,
                                            'label' => "Arresto cardiaco o dolori toracici",
                                        ],
                                        [
                                            "order" => 14,
                                            'label' => "Ictus",
                                        ],
                                        [
                                            "order" => 15,
                                            'label' => "Lesioni cerebrali e spinali",
                                        ],
                                        [
                                            "order" => 16,
                                            'label' => "Rimozione della protezione per la testa",
                                        ],
                                        [
                                            "order" => 17,
                                            'label' => "Rotazione del paziente in asse",
                                        ],
                                        [
                                            "order" => 18,
                                            'label' => "Preparazione di un tutore per un braccio",
                                        ],
                                        [
                                            "order" => 19,
                                            'label' => "Preparazione di una steccatura",
                                        ],
                                        [
                                            "order" => 20,
                                            'label' => "Arresto respiratorio compreso annegamento",
                                        ],
                                        [
                                            "order" => 21,
                                            'label' => "Compressioni toraciche (Massaggio cardiaco)",
                                        ],
                                        [
                                            "order" => 22,
                                            'label' => "Respirazioni di soccorso (Bocca a bocca)",
                                        ],
                                        [
                                            "order" => 23,
                                            'label' => "Rianimazione cardiopolmonare (BLS)",
                                        ],

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "React Right - AED",
            "code" => "RR2",
            'cost' => 42.70,
            "price" => 130.00,
            "discounted_price" => 90.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Sessioni di apprendimento attivo',
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Uso di un defrillibatore automatico esterno',
                                        ],

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "React Right - O2",
            "code" => "RR3",
            'cost' => 42.700,
            "price" => 150.00,
            "discounted_price" => 100.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Sessioni di apprendimento attivo',
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => "Preparazione dell'unità di erogazione dell'ossigeno",
                                        ],
                                        [
                                            "order" => 2,
                                            'label' => "Smontaggio dell'unità di erogazione dell'ossigeno",
                                        ],
                                        [
                                            "order" => 3,
                                            'label' => "Somministrazione di ossigeno ad una vittima cosciente, che respira (maschera a domanda)",
                                        ],
                                        [
                                            "order" => 4,
                                            'label' => "Somministrazione di ossigeno ad una vittima incosciente, che respira (maschera Non-Rebreather)",
                                        ],
                                        [
                                            "order" => 5,
                                            'label' => "Somministrazione di ossigeno ad una vittima incosciente, che non respira (Pocket Mask)",
                                        ],
                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 3,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Photo and Video",
            "code" => "PV",
            'cost' => 43.80,
            "price" => 215.00,
            "discounted_price" => 160.00,
            "is_speciality" => true,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],

                [
                    'label' => 'OW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Preparazione della fotocamera"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua - avendo con sé la fotocamera",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto neutro (nella posizione d'immersione)"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto neutro (a mezz'acqua)"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Muoversi con una fotocamera in immersione"
                                        ],
                                        [
                                            "order" => 6,
                                            "label" => "Fare pratica con la fotocamera"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi post-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (fotocamera)"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [

                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Preparazione della fotocamera"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua - avendo con sé la fotocamera",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto, con indossato il Sistema Totale d'Immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Assetto neutro (nella posizione d'immersione)"
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto neutro (a mezz'acqua)"
                                        ],
                                        [
                                            "order" => 5,
                                            "label" => "Muoversi con una fotocamera in immersione"
                                        ],
                                        [
                                            "order" => 6,
                                            "label" => "Fare pratica con la fotocamera"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi post-immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (fotocamera)"
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],

            ]
        ],
    ];



    /*

    [
            "name" => "Extended Range Foundations",
            "code" => "ERF",
            'cost' => 20.62,
            "price" => 300.00,
            "discounted_price" => 250.00
        ],
        [
            "name" => "Extended Range Nitrox Diving",
            "code" => "ERN",
            'cost' => 43.80,
            "price" => 470.00,
            "discounted_price" => 3560.00
        ],
        [
            "name" => "Extended Range",
            "code" => "ER",
            'cost' => 57.22,
            "price" => 500.00,
            "discounted_price" => 380.00
        ],
        [
            "name" => "Technical Extended Range",
            "code" => "TER",
            'cost' => 57.22,
            "price" => 500.00,
            "discounted_price" => 380.00,
            "is_speciality" => false,
            "own_speciality" => false,
            "activities" => [
                [
                    'label' => 'THEORY',
                    'order' => 1,
                    'values' => [
                        [
                            'label' => 'Ripassi delle conoscenze',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Ripassi delle conoscenze',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                        [
                            'label' => 'Esame',
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            'label' => 'Esame',
                                        ]

                                    ]
                                ]
                            ],
                        ],
                    ]
                ],
                [
                    'label' => 'CW',
                    'order' => 2,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Assemblaggio dell'attrezzatura"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Rimozione e riposizionamento della bombola di fase (in superficie)"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Esercizio S (in acqua bassa)"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Rimozione e riposizionamento dell'attrezzatura di back-up",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Assetto a mezz'acqua (Hovering) e trim",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Pinneggiata alternata e pinneggiata alternata modificata",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Rana e rana modificata",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Rana all'indietro",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Utilizzo delle torce (facoltativo)",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Calcolo del consumo sul fondo",
                                        ],
                                        [
                                            "order" => 10,
                                            "label" =>
                                            "Cedimento della maschera principale",
                                        ],
                                        [
                                            "order" => 11,
                                            "label" =>
                                            "Subacqueo senza gas (in assetto ed in risalita)",
                                        ],
                                        [
                                            "order" => 12,
                                            "label" =>
                                            "Dispiegamento di una cima guida statica",
                                        ],
                                        [
                                            "order" => 13,
                                            "label" =>
                                            "Rimozione e riposizionamento della bombola di fase (in immersione)",
                                        ],
                                        [
                                            "order" => 14,
                                            "label" =>
                                            "Impigliamento",
                                        ],
                                        [
                                            "order" => 15,
                                            "label" =>
                                            "Rimozione e riposizionamento della bombola di fase (nuotando)",
                                        ],
                                        [
                                            "order" => 16,
                                            "label" =>
                                            "Scambio delle bombole di fase",
                                        ],
                                        [
                                            "order" => 17,
                                            "label" =>
                                            "Comando di carico principale in erogazione continua",
                                        ],
                                        [
                                            "order" => 18,
                                            "label" =>
                                            "Approccio globale alla chiusura (diagnosi del problema)",
                                        ],
                                        [
                                            "order" => 19,
                                            "label" =>
                                            "Avaria del sistema principale di controllo dell'assetto (muta stagna, GAV di ridondanza o pallone di sollevamento)",
                                        ],
                                        [
                                            "order" => 20,
                                            "label" =>
                                            "Passaggio al gas decompressivo",
                                        ],
                                        [
                                            "order" => 21,
                                            "label" =>
                                            "Erogatore della bombola di fase in erogazione continua (apertura e chiusura della rubinetteria)",
                                        ],
                                        [
                                            "order" => 22,
                                            "label" =>
                                            "Soccorso per tossicità dell'ossigeno (in immersione)",
                                        ],
                                        [
                                            "order" => 23,
                                            "label" =>
                                            "Subacqueo inconscio (in immersione)",
                                        ],
                                        [
                                            "order" => 24,
                                            "label" =>
                                            "Conduzione di un piano decompressivo",
                                        ],
                                        [
                                            "order" => 25,
                                            "label" =>
                                            "Lancio del segnalatore di superficie (SMB)",
                                        ],
                                        [
                                            "order" => 26,
                                            "label" =>
                                            "Subacqueo inconscio (in superficie)",
                                        ],
                                    ]
                                ],

                                [
                                    "label" => "Debriefing",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
                [
                    'label' => 'OW',
                    'order' => 3,
                    'values' => [
                        [
                            'order' => 1,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione dell'unità SCUBA con indosso la stagna"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto con il Sistema Totale d'Immersione completo, con indosso la stagna"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata (Muta stagna)",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Confidenza con la stagna",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Esercizi con le valvole della stagna - Esercizi di malfunzionamento della valvola",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Subacqueo a testa in giù",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Eccessivo gonfiaggio di una muta stagna",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Simulazione dell'allagamento della stagna",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Risalita controllata (Muta stagna)",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (Muta stagna)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                        [
                            'order' => 2,
                            'values' => [
                                [
                                    "label" => "Esercizi pre-immersione",
                                    "order" => 1,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Pianificazione dell'immersione"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Vestizione dell'unità SCUBA con indosso la stagna"
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Check pre-immersione"
                                        ]
                                    ]
                                ],
                                [
                                    "label" => "Esercizi in acqua",
                                    "order" => 2,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Controllo dell'assetto con il Sistema Totale d'Immersione completo, con indosso la stagna"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" => "Discesa controllata (Muta stagna)",
                                        ],
                                        [
                                            "order" => 3,
                                            "label" => "Confidenza con la stagna",
                                        ],
                                        [
                                            "order" => 4,
                                            "label" => "Esercizi con le valvole della stagna - Esercizi di malfunzionamento della valvola",
                                        ],
                                        [
                                            "order" => 5,
                                            "label" =>
                                            "Assetto neutro (a mezz'acqua)",
                                        ],
                                        [
                                            "order" => 6,
                                            "label" =>
                                            "Subacqueo a testa in giù",
                                        ],
                                        [
                                            "order" => 7,
                                            "label" =>
                                            "Eccessivo gonfiaggio di una muta stagna",
                                        ],
                                        [
                                            "order" => 8,
                                            "label" =>
                                            "Simulazione dell'allagamento della stagna",
                                        ],
                                        [
                                            "order" => 9,
                                            "label" =>
                                            "Risalita controllata (Muta stagna)",
                                        ],

                                    ]
                                ],
                                [
                                    "label" => "Esercizi dopo l'immersione",
                                    "order" => 3,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Cura dell'attrezzatura (Muta stagna)"
                                        ],
                                    ]
                                ],
                                [
                                    "label" => "Debriefing",
                                    "order" => 4,
                                    "values" => [
                                        [
                                            "order" => 1,
                                            "label" => "Debriefing"
                                        ],
                                        [
                                            "order" => 2,
                                            "label" =>
                                            "Registrare e convalidare immersione su Logbook App SSI",
                                        ],
                                    ]
                                ],
                            ]
                        ],
                    ]
                ],
            ]
        ],
        [
            "name" => "Hypoxic Trimix (100m)",
            "code" => "HER",
            'cost' => 57.22,
            "price" => 550.00,
            "discounted_price" => 400.00
        ],
        [
            "name" => "Extended Range Wreck Diving",
            "code" => "HER",
            'cost' => 57.22,
            "price" => 470.00,
            "discounted_price" => 350.00
        ],
        [
            "name" => "Technical Wreck Diving",
            "code" => "TWD",
            'cost' => 57.22,
            "price" => 470.00,
            "discounted_price" => 350.00
        ],
        */
